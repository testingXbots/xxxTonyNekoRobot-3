...
async def rm_deletedacc(show):
    ...
    if con != "clean":
        kontol = await show.reply("`Searching for deleted accounts to fu*k...`")
        ...
        if del_u > 0:
            del_status = (
                f"**{del_u} Deleted account(s) found in this group,"
    ...
    async for user in telethn.iter_participants(show.chat_id):
        if user.deleted:
            ...
            del_u += 1
    if del_u > 0:
        del_status = f"**Kicked {del_u} Deleted account(s)**"
    if del_a > 0:
        del_status = (
